...
from misc.share_array import get_shared_array, make_shared_array
...
# 2D-stacked smart erode multi-threaded
large_window_size=5
small_window_size=1
outCombined = np.zeros(tiffarray.shape, dtype=np.uint8)

# outCombined_shared = mp.Array(ctypes.c_uint8, tiffarray.shape[0] * tiffarray.shape[1] * tiffarray.shape[2])

# outCombined = np.frombuffer(outCombined_shared.get_obj())
make_shared_array(tiffarray, name="tiffarray")
make_shared_array(outCombined, name='outCombined')  


def smart_erode_2d(slice_num):
    tiffarray = get_shared_array('tiffarray')
    slice = tiffarray[:,:,slice_num]
    out2D = slice.copy()
    ...
    #         window = slice[np.ix_(rows, columns)]
    ...
    #             small_window = slice[np.ix_(rows, columns)]            
    #             out2D[x][y] = np.min(small_window)
    
    outCombinedShared = get_shared_array('outCombined')
    outCombinedShared[slice] = out2D
    # with outCombined_shared.get_lock():
    #     # outCombined_shared[slice] = out2D
    #     arr = np.frombuffer(outCombined_shared.get_obj()) # no data copying
    #     arr[slice_num * 512: (slice+1) * 512] = out2D



with Pool(processes=3) as pool:
    _ = pool.map(smart_erode_2d, range(tiffarray.shape[0]))  # use a multiprocessing.Pool to distribute work to worker processes
